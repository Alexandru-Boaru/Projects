...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0.9, 0, 0));
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab8"], modelMatrix, glm::vec3(0, 0, 1));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1,1,0));
	}
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_UP)) lightAngleX += deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_DOWN)) lightAngleX -= deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_LEFT)) lightAngleY -= deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_RIGHT)) lightAngleY += deltaTime * speed;

		if (window->KeyHold(GLFW_KEY_Z)) cut_off -= deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_X)) cut_off += deltaTime * speed;
