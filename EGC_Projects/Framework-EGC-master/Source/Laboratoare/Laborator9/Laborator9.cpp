...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, 0,1,mapTextures["earth"]);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, 0,0, mapTextures["crate"]);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, 0,0, mapTextures["random"]);
	}

	{
		...
		modelMatrix = glm::rotate(modelMatrix, grassAngle, glm::vec3(0, 1, 0));
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, 0,0, mapTextures["grass"]);




		modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0f, 1.5f, 1.0f));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, 1,0, mapTextures["grass"], mapTextures["random"]);
	}

	{
		...
		RenderSimpleMesh(meshes["bamboo"], shaders["ShaderLab9"], modelMatrix, 0,0, mapTextures["bamboo"]);
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, int useSecond, int rotation, Texture2D* texture1, Texture2D* texture2)
{
	...
	int use_second = glGetUniformLocation(shader->program, "use_second");
	glUniform1i(use_second, useSecond);

	int r = glGetUniformLocation(shader->program, "rotation");
	glUniform1i(r, rotation);
